...
void SoundManager::StopAll()
{
   for (unsigned int i = 0; i < MAX_SIMULTANEOUS_SOUNDS; i++)
   {
      ...
      if ((pInProgressSound->mSFSound.getStatus() == sf::SoundSource::Playing))
...
void SoundManager::Pause(Sound sound)
{
   for (unsigned int i = 0; i < MAX_SIMULTANEOUS_SOUNDS; i++)
   {
      ...
      if ((pInProgressSound->mSound == sound) &&
          (pInProgressSound->mSFSound.getStatus() == sf::SoundSource::Playing))
         pInProgressSound->mSFSound.pause();
   }
}

void SoundManager::UnPause(Sound sound)
{
   for (unsigned int i = 0; i < MAX_SIMULTANEOUS_SOUNDS; i++)
   {
      InProgressSound* pInProgressSound = &mInProgressSound[i];
      ...
      if ((pInProgressSound->mSound == sound) &&
          (pInProgressSound->mSFSound.getStatus() == sf::SoundSource::Paused))
         pInProgressSound->mSFSound.play();
   }
}
...
void SoundManager::UnPauseAll()
{
   for (unsigned int i = 0; i < MAX_SIMULTANEOUS_SOUNDS; i++)
   {
      ...
      if ((pInProgressSound->mSFSound.getStatus() == sf::SoundSource::Paused))
         pInProgressSound->mSFSound.play();
   }
}