...
public:
   ...
   template <typename T>
   bool Collision(bool includeDying = false)
   {
      bool found = false;
   ...
   template <typename T>
   bool DetectWithinRadius(float radius, std::vector<T*>& characters, bool includeDying = false)
   {
      bool found = false;
         ...
         it++)
      {
            ...
            ((*it)->GetStatus() == alive) || (includeDying && ((*it)->GetStatus() == dying)))
         {
            ...
            unsigned int dx = static_cast<unsigned int>(fabs(mX - (*it)->mX));
            unsigned int dy = static_cast<unsigned int>(fabs(mY - (*it)->mY));
            if ((dx < radius + 0.1f) &&
                (dy < radius + 0.1f) &&
                (((*it)->mX - mX) * ((*it)->mX - mX) + ((*it)->mY - mY) * ((*it)->mY - mY)) < (radius * radius))
            {
               characters.push_back(reinterpret_cast<T*>(*it));
               found = true;
            }
